...
var runGalen = function (stream, cb, galenPath, mode, file, cwd, opt, properties) {
    ...
    args.push('-Dwebdriver.chrome.driver=' + resolveNodePath('chromedriver', 'lib/chromedriver/chromedriver'+ (process.platform === 'win32' ? '.exe' : '')));
    args.push('-Dwebdriver.gecko.driver=' + resolveNodePath('geckodriver', 'geckodriver'+ (process.platform === 'win32' ? '.exe' : '')));
